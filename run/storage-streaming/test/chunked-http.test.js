...
describe('chunked http tests', () => {
  it('should upload file to GCS', async () => {
    ...
    //   exec's 'timeout' param won't kill children of "shim" /bin/sh process
    //   Workaround: include "& sleep <TIMEOUT>; kill $!" in executed command
    const proc = execPromise(makeFFCmd('chunkedUpload', PORT), {shell: true, cwd});
    ...
    const [exists] = await storage.bucket(FUNCTIONS_BUCKET).file(copiedFilename).exists();
  ...
  it('should download file', async () => {
    ...
    const proc = execPromise(makeFFCmd('chunkedDownload', PORT), {shell: true, cwd});
